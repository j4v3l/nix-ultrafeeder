...
in {
  options.services.adsbFeeders.flightradar24 = {
    ...
    imageFile = lib.mkOption {
      ...
      default = null;
      description = "Optional local image tarball for the FlightRadar24 container.";
    };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.docker-flightradar24 = lib.mkIf (cfg.networkName != null) {
      serviceConfig = {
        Path = [pkgs.coreutils pkgs.docker pkgs.podman];
        ExecStartPre = [
          (pkgs.writeShellScript "ensure-fr24-net" ''
            set -euo pipefail
            net="${cfg.networkName}"
            [ -z "$net" ] && exit 0
            tool=""
