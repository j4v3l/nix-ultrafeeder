{
  ...
  cfg = config.services.adsbFeeders.adsbhub;
in {
  options.services.adsbFeeders.adsbhub = {
    meta = {
      maintainers = ["j4v3l"];
      description = "Run SDR-Enthusiasts ADSBHub feeder container";
    };
    enable = lib.mkEnableOption "Run ADSBHub feeder container";
    ...
    image = lib.mkOption {
      ...
      description = "ADSBHub feeder image repository.";
    };

    tag = lib.mkOption {
      type = lib.types.str;
      default = "latest";
      description = "ADSBHub feeder image tag.";
    };

    imageFile = lib.mkOption {
      type = lib.types.nullOr lib.types.path;
      default = null;
      description = "Optional local image tarball for the ADSBHub container.";
    };
    ...
    environment = lib.mkOption {
      ...
      default = {};
      description = "Extra environment variables passed to the ADSBHub container.";
    };

    environmentFiles = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [];
      description = "Environment files passed to the ADSBHub container (recommended for secrets).";
    };

    extraOptions = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [];
      description = "Additional docker/podman CLI options for the ADSBHub container.";
    };
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.docker-adsbhub = lib.mkIf (cfg.networkName != null) {
      serviceConfig = {
        Path = [pkgs.coreutils pkgs.docker pkgs.podman];
        ExecStartPre = [
          (pkgs.writeShellScript "ensure-adsbhub-net" ''
            set -euo pipefail
            net="${cfg.networkName}"
            [ -z "$net" ] && exit 0
            tool=""
